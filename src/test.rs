...
fn reduced_qc_example() {
    ...
    for _ in 0..total_elements {
        ...
        assert!(
            !seen.contains(&(a, b, c)),
            "Returned an element already seen"
        );
        seen.insert((a, b, c));
        assert!(va.contains(a), "`a` not in `A`");
        assert!(vb.contains(b), "`b` not in `B`");
        assert!(vc.contains(c), "`c` not in `C`");
    }
    assert!(
        iter.next().is_none(),
        "Kept returning after should have returned `None`"
    );
}
...
mod qc {
    ...
    quickcheck::quickcheck! {
        fn prop_everything(va: Vec<A>, vb: Vec<B>, vc: Vec<C>, a0: A, b0: B, c0: C) -> bool {
            ...
            let total_elements = va.len() * vb.len() * vc.len();
            let mut seen = ::std::collections::HashSet::new();
            let mut iter = (va.iter(), vb.iter(), vc.iter()).breadth_first();
            for _ in 0..total_elements {
